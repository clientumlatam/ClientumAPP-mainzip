...
**How to apply:** New provider integrations should add fields to the module catalog, resolve them on the backend by verified user identity, and never persist raw values in the browser. Production requests require Firebase identity verification; the local user-id header is demo-only. For cross-user API Key administration, use a server-side admin allowlist in production and only the demo role fallback in development.